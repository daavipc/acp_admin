<? if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			if($insert) {
				$core->logger("O usuário adicionou uma nova coluna.", "acao");
...
<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			if($update) {
				$core->logger("O usuário editou a coluna [#$id].", "acao");
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			if(!in_array($id_atual, $ids_forbidden)) {
				...
				$core->logger("O usuário deletou uma coluna [#$id_atual]", "acao");
			} else {
		...
		$delete->execute();
			...
			$core->logger("O usuário deletou uma coluna [#$id]", "acao");
		} else {
...
<div class="box-content">
	...
	while($sql2 = $sql->fetch()) {
		$table->insertBody(array($sql2['id'], $core->clear($sql2['nome']), $sql2['autor'], $core->clear(date('d/m/Y H:i', $sql2['data'])), 'actions'), $sql2['status']);
	}
