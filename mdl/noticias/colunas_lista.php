<? if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			if($insert) {
				$core->logger("O usuário adicionou uma nova coluna (lista).", "acao");
...
<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			if($update) {
				$core->logger("O usuário editou a coluna [#$id] (lista).", "acao");
...
<? if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário deletou a coluna [#$id_atual] (lista)", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$core->logger("O usuário deletou a coluna [#$id] (lista)", "acao");
	}
} ?>
...
<div class="box-content">
	...
	$table->head(array('#', 'Nome', 'Edições postadas', 'Data', 'Ações'));
	...
	while($sql2 = $sql->fetch()) {
		$sql3 = $conn->query("SELECT count(id) FROM noticias WHERE col_id='".$sql2['id']."'");
