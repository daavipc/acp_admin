<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			if($update) {
				$core->logger("O usuário editou o comentário de notícia [#$id].", "acao");
...
<? if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário deletou o comentário de notícia [#$id_atual]", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$core->logger("O usuário deletou o comentário de notícia [#$id]", "acao");
	}
} ?>
...
<div class="box-content">
	...
	$table->head(array('#', 'Conteúdo', 'Notícia', 'Autor', 'Data', 'Ações'));
	...
	while($sql2 = $sql->fetch()) {
		$sql3 = $conn->prepare("SELECT titulo, id FROM noticias WHERE id = ?");
		$sql3->bindValue(1, $sql2['id_noticia']);
		$sql3->execute();
		$sql4 = $sql3->fetch();
