<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			if($update) {
				$core->logger("O usuário editou o post [#$id].", "acao");
...
<? if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário deletou o post [#$id_atual]", "acao");
		}
	} else {
		$sql = $conn->prepare("SELECT autor FROM $mdl_tabela WHERE id = ?");
		$sql->bindValue(1, $id);
		...
		$core->logger("O usuário deletou o post [#$id]", "acao");
	}
} ?>
...
<div class="box-content">
	...
	$table->head(array('#', 'Conteúdo', 'Tópico', 'Autor', 'Data', 'Ações'));
	...
	while($sql2 = $sql->fetch()) {
		$sql3 = $conn->prepare("SELECT titulo, id FROM forum_topicos WHERE id = ?");
		$sql3->bindValue(1, $sql2['id_topico']);
		$sql3->execute();
		$sql4 = $sql3->fetch();
		$table->insertBody(array($sql2['id'], $core->clear($core->encurtar($sql2['conteudo'], 50)), '<a href="/forum/topicos/'.$sql4['id'].'/'.$core->trataurl($sql4['titulo']).'">'.$core->clear($sql4['titulo']).'</a>', $core->clear($sql2['autor']), $core->clear(date('d/m/Y H:i', $sql2['data'])), 'actions'), $sql2['status']);
	}
