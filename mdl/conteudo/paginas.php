...
if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		$equipe = $core->clear($_POST['equipe']);
		$_senhaa = $core->clear($_POST['senha']);
		$prosseguir = true;

		if($equipe) { $equipe = 's'; } else { $equipe = 'n'; }
		...
		if($prosseguir) {
			...
			$insert_data['equipe'] = $equipe;
			$insert_data['senha'] = $_senhaa;
			...
			if($insert) {
				$core->logger("O usuário adicionou uma nova página.", "acao");

				$sql3 = $conn->prepare("SELECT id, titulo FROM paginas WHERE autor = ? ORDER BY id DESC LIMIT 1");
				$sql3->bindValue(1, $core->autor);
				$sql3->execute();
				$sql4 = $sql3->fetch();

				$link = DOMAIN . '/paginas/' . $sql4['id'] . '/' . $core->trataurl($sql4['titulo']);
...
<div class="box-content">
	...
	$form->createCheckbox('Página registra a equipe', 'equipe');
	$form->createInput('Página com senha', 'text', 'senha', '', '', '', 'Deixe em branco para a página ser livre.');
...
<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			$update_data['equipe'] = $equipe;
			$update_data['senha'] = $_senhaa;

			$where_data['id'] = $id;
			$update = $sqlActions->update($mdl_tabela, $update_data, $where_data);

			if($update) {
				$core->logger("O usuário editou a página [#$id].", "acao");

				$inInt['id_pagina'] = $id;
				$inInt['ato'] = 'O usuário editou a página.';
				$inInt['autor'] = $core->autor;
				$inInt['data'] = $core->timestamp;

				$insInt = $sqlActions->insert("paginas_interacoes", $inInt);
...
<div class="box-content">
	...
	$link = DOMAIN . '/paginas/' . $ex['id'] . '-' . $core->trataurl($ex['titulo']);
	...
	$form->createCheckbox('Página registra a equipe', 'equipe', (($ex['equipe'] == 's')) ? true : false);
	$form->createInput('Página com senha', 'text', 'senha', $ex['senha'], '', '', 'Deixe em branco para a página ser livre.');
...
<? if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário deletou a página [#$id_atual]", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$core->logger("O usuário deletou a página [#$id]", "acao");
	}
} ?>
