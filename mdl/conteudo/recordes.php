<? if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		if(empty($titulo) || empty($descricao) || empty($conteudo) || empty($usuario)) {
		...
		if($prosseguir) {
			...
			$insert_data['resumo'] = $descricao;
			$insert_data['texto'] = $conteudo;
			$insert_data['usuario'] = $usuario;
			...
			if($insert) {
				$core->logger("O usuário adicionou um novo recorde.", "acao");
...
<div class="box-content">
	...
	$form->createTextarea('Texto', 'conteudo', '', 'ckeditor','ckeditor');
	$form->createUpload('Imagem', 'imagem');
	$form->createInput('Usuário detentor do recorde', 'text', 'usuario');
...
<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			$update_data['resumo'] = $descricao;
			$update_data['texto'] = $conteudo;
			...
			if($update) {
				$core->logger("O usuário editou o recorde [#$id].", "acao");
...
<div class="box-content">
	...
	$form->createInput('Descrição', 'text', 'descricao', $ex['resumo'], 'no-label s-lg');
	$form->createTextarea('Texto', 'conteudo', $ex['texto'], 'ckeditor','ckeditor');
	$form->createUpload('Imagem', 'imagem', $ex['imagem']);
	$form->createInput('Usuário detentor do recorde', 'text', 'usuario', $ex['usuario']);
...
<? if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário deletou o recorde [#$id_atual]", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$core->logger("O usuário deletou o recorde [#$id]", "acao");
	}
} ?>
...
<div class="box-content">
	...
	$table->head(array('#', 'Título', 'Descrição', 'Data', 'Ações'));
	...
	while($sql2 = $sql->fetch()) {
		$table->insertBody(array($sql2['id'], $core->clear($sql2['titulo']), $core->clear($sql2['resumo']), $core->clear(date('d/m/Y H:i', $sql2['data'])), 'actions'), $sql2['status']);
	}
