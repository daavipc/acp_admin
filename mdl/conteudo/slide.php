<? if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		if(empty($descricao) || empty($link) || empty($titulo)) {
		...
		if($prosseguir) {
			...
			if($insert) {
				$core->logger("O usuário adicionou um novo slide.", "acao");
...
<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			if($update) {
				$core->logger("O usuário editou o slide [#$id].", "acao");
...
<? if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário deletou o slide [#$id_atual]", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$core->logger("O usuário deletou o slide [#$id]", "acao");
	}
} ?>
...
<div class="box-content">
	...
	$table->head(array('#', 'Título', 'Cliques', 'Data', 'Ações'));
	...
	while($sql2 = $sql->fetch()) {
		$sql3 = $conn->query("SELECT count(id) FROM slide_clicks WHERE id_slide='".$sql2['id']."'");
		$table->insertBody(array($sql2['id'], $core->clear($sql2['titulo']), '<fx>' . $sql3->fetchColumn() . '</fx>', $core->clear(date('d/m/Y H:i', $sql2['data'])), 'actions'), $sql2['status']);
	}
