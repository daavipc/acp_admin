<? if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		$id_menu = $core->clear($_POST['menu_pai']);
		$prosseguir = true;
		...
		if($prosseguir) {
			...
			if($insert) {
				$core->logger("O usuário adicionou um novo sub-menu.", "acao");
...
<div class="box-content">
	...
	$form->createInput('Link', 'text', 'link', '', '', '', 'Evite utilizar caminhos absolutos. Utilize caminhos relativos.<br>Caminho absoluto - <pre>http://www.icehabbo.com.br/icehabbo/equipe</pre><br>Caminho relativo - <pre>/icehabbo/equipe</pre>');

	$menus = array();
	$sql = $conn->query("SELECT * FROM menu WHERE link = '' ORDER BY id ASC");
	while($sql2 = $sql->fetch()) {
		$atual = array("label" => $core->clear($sql2['nome']), "value" => $sql2['id']);
		$menus[] = $atual;
	}

	$form->createSelect('Menu pai', 'menu_pai', $menus);
...
<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			$update_data['id_menu'] = $id_menu;

			$where_data['id'] = $id;
			$update = $sqlActions->update($mdl_tabela, $update_data, $where_data);

			if($update) {
				$core->logger("O usuário editou o sub-men [#$id].", "acao");
...
<div class="box-content">
	...
	$form->createInput('Link', 'text', 'link', $ex['link'], '', '', 'Evite utilizar caminhos absolutos. Utilize caminhos relativos.<br>Caminho absoluto - <pre>http://www.icehabbo.com.br/icehabbo/equipe</pre><br>Caminho relativo - <pre>/icehabbo/equipe</pre>');
	...
	$form->createSelect('Menu pai', 'menu_pai', $menus, $ex['id_menu']);
...
<div class="box-content">
	...
	$table->head(array('#', 'Nome', 'Menu pai', 'Data', 'Ações'));
	...
	while($sql2 = $sql->fetch()) {
		$sql3 = $conn->prepare("SELECT * FROM menu WHERE id = ?");
		$sql3->bindValue(1, $sql2['id_menu']);
		$sql3->execute();
		$sql4 = $sql3->fetch();
		$table->insertBody(array($sql2['id'], $core->clear($sql2['nome']), $core->clear($sql4['nome']), $core->clear(date('d/m/Y H:i', $sql2['data'])), 'actions'), $sql2['status']);
	}
