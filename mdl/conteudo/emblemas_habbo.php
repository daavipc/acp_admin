<? if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		$numeracao = $core->clear($_POST['numeracao']);
		$titulo = $core->clear($_POST['titulo']);
		$descricao = $core->clear($_POST['descricao']);
		$prosseguir = true;

		if(empty($sigla) || empty($numeracao)) {
			$form_return .= aviso_red("Preencha todos os campos.");
			$prosseguir = false;
		}

		$sql = $conn->prepare("SELECT count(id) FROM emblemas_habbo WHERE sigla = ? AND numeracao = ?");
		$sql->bindValue(1, $sigla);
		$sql->bindValue(2, $numeracao);
		$sql->execute();
		$rows = $sql->fetchColumn();

		if($rows > 0) {
			$form_return .= aviso_red("Este emblema já está cadastrado.");
			$prosseguir = false;
		}

		if($prosseguir) {
			$insert_data['sigla'] = $sigla;
			$insert_data['numeracao'] = $numeracao;
			...
			if($insert) {
				$core->logger("O usuário adicionou um novo emblema habbo.", "acao");
...
<div class="box-content">
	...
	$form->createInput('Sigla do Hotel', 'text', 'sigla');
	$form->createInput('Numeração do emblema', 'text', 'numeracao');
...
<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		$sql = $conn->prepare("SELECT count(id) FROM emblemas_habbo WHERE sigla = ? AND numeracao = ? AND id != ?");
		$sql->bindValue(1, $sigla);
		$sql->bindValue(2, $numeracao);
		$sql->bindValue(3, $id);
		$sql->execute();
		...
		if($prosseguir) {
			...
			$update_data['numeracao'] = $numeracao;
			...
			if($update) {
				$core->logger("O usuário editou o emblema do habbo [#$id].", "acao");
...
<div class="box-content">
	...
	$form->createInput('Sigla do Hotel', 'text', 'sigla', $ex['sigla']);
	$form->createInput('Numeração do emblema', 'text', 'numeracao', $ex['numeracao']);
...
<? if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário inativou o emblema do Habbo [#$id_atual]", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$core->logger("O usuário inativou o emblema do Habbo [#$id]", "acao");
	}
} ?>
...
<div class="box-content">
	...
	$table->head(array('#', 'Código', 'Título', 'Imagem', 'Autor', 'Ações'));
	...
	while($sql2 = $sql->fetch()) {
		$table->insertBody(array($sql2['id'], $core->clear($sql2['sigla'] . $sql2['numeracao']), $core->clear($sql2['titulo']), '<img src="http://images.habbo.com/c_images/album1584/'.$core->clear($sql2['sigla'] . $sql2['numeracao']).'.gif">', $core->clear(date('d/m/Y H:i', $sql2['data'])), 'actions'), $sql2['status']);
	}
