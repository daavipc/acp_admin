<? if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		$proprietario = $core->clear($_POST['proprietario']);
		$quantidade = $core->clear($_POST['quantidade']);
		$link = $core->clear($_POST['link']);
		$prosseguir = true;

		if(empty($nome) || empty($proprietario) || empty($link)) {
		...
		if($prosseguir) {
			...
			$upload = new Upload($conn, $up_gallery, $up_file, $up_url, 'rooms-', true);
		...
		if($prosseguir) {
			...
			$insert_data['proprietario'] = $proprietario;
			$insert_data['imagem'] = $caminho_img;
			$insert_data['quantidade'] = $quantidade;
			...
			if($insert) {
				$core->logger("O usuário adicionou um novo quarto recomendado.", "acao");
...
<div class="box-content">
	...
	$form->createInput('Proprietário', 'text', 'proprietario');
	$form->createInput('Quantidade de Habbos', 'text', 'quantidade');
...
<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			$upload = new Upload($conn, $up_gallery, $up_file, $up_url, 'rooms-', false, $ex['imagem']);
		...
		if($prosseguir) {
			...
			$update_data['proprietario'] = $proprietario;
			...
			if($update) {
				$core->logger("O usuário editou o quarto recomendado [#$id].", "acao");
...
<div class="box-content">
	...
	$form->createInput('Proprietário', 'text', 'proprietario', $ex['proprietario']);
	$form->createInput('Quantidade de Habbos', 'text', 'quantidade', $ex['quantidade']);
...
<? if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário deletou o quarto recomendado [#$id_atual]", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$core->logger("O usuário deletou o quarto recomendado [#$id]", "acao");
	}
} ?>
