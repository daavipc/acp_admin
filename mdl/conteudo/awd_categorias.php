<? if($_GET['a'] == 1) {
    if($_POST['form'] == 'form') {
        ...
        if($prosseguir) {
            ...
            if($insert) {
                $core->logger("O usuário adicionou uma nova categoria no Ice Awards.", "acao");
...
<? if($_GET['a'] == 2) {
    ...
    if($_POST['form'] == 'form') {
        ...
        if($prosseguir) {
            ...
            if($update) {
                $core->logger("O usuário editou uma categoria do Ice Awards [#$id].", "acao");
...
<div class="box-content">
    ...
    while($sql2 = $sql->fetch()) {
        $table->insertBody(array($sql2['id'], $core->clear($sql2['nome']), $core->clear(date('d/m/Y H:i', $sql2['data'])), 'actions'), $sql2['status']);
    }
