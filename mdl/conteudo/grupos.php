<? if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			$upload = new Upload($conn, $up_gallery, $up_file, $up_url, 'groups-', true);
		...
		if($prosseguir) {
			...
			if($insert) {
				$core->logger("O usuário adicionou um novo grupo recomendado.", "acao");
...
<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if(empty($nome) || empty($descricao) || empty($link)) {
			$form_return .= aviso_red("Preencha todos os campos.");
			$prosseguir = false;
		}
		...
		if($prosseguir) {
			...
			if($update) {
				$core->logger("O usuário editou o grupo recomendado [#$id].", "acao");
...
<? if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário deletou o grupo recomendado [#$id_atual]", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$core->logger("O usuário deletou o grupo recomendado [#$id]", "acao");
	}
} ?>
