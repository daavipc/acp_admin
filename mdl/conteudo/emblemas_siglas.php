<? if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			$insert_data['sigla'] = $sigla;
			$insert_data['cod_ultimo'] = $codigo;
			...
			if($insert) {
				$core->logger("O usuário adicionou uma nova sigla nos Emblemas Habbo.", "acao");
...
<div class="box-content">
	...
	$form->createInput('Sigla', 'text', 'sigla');
	$form->createInput('Último emblema descoberto', 'text', 'codigo', '', 'w-md', '', 'Digite o código do último emblema descoberto desta sigla. (somente a numeração)');
...
<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			$update_data['sigla'] = $sigla;
			$update_data['cod_ultimo'] = $codigo;

			$where_data['id'] = $id;
			$update = $sqlActions->update($mdl_tabela, $update_data, $where_data);

			if($update) {
				$core->logger("O usuário editou a sigla de Emblema Habbo [#$id].", "acao");
...
<div class="box-content">
	...
	$form->createInput('Sigla', 'text', 'sigla', $ex['sigla']);
	$form->createInput('Último emblema descoberto', 'text', 'codigo', $ex['cod_ultimo'], 'w-md', '', 'Digite o código do último emblema descoberto desta sigla. (somente a numeração)');
...
<? if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário deletou a sigla de emblema habbo [#$id_atual]", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$core->logger("O usuário deletou a sigla de emblema habbo [#$id]", "acao");
	}
} ?>
...
<div class="box-content">
	...
	$table->head(array('#', 'Sigla', 'Último emblema descoberto', 'Data', 'Ações'));
	...
	while($sql2 = $sql->fetch()) {
		$table->insertBody(array($sql2['id'], $core->clear($sql2['sigla']), $core->clear($sql2['cod_ultimo']), $core->clear(date('d/m/Y H:i', $sql2['data'])), 'actions'), $sql2['status']);
	}
...
<div class="box-content">
	<div class="title-section">Últimos 150 emblemas descobertos</div>

	<? $sql3 = $conn->query("SELECT * FROM emblemas_habbo ORDER BY id DESC LIMIT 150");
	while($sql4 = $sql3->fetch()) {
		echo '<div style="border:1px solid #AAA;border-radius:4px;padding:5px;float:left;margin:3px;">';
		echo $sql4['sigla'] . $sql4['numeracao'] . ' <img src="http://images.habbo.com/c_images/album1584/'.$sql4['sigla'] . $sql4['numeracao'].'.gif"><br>';
		echo '</div>';
	}
