<? if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		$preco_moedas = $_POST['preco_moedas'];
		...
		if(!is_numeric($preco_moedas) || $preco_moedas < 0) {
			$form_return .= aviso_red("O Ppreço moedas digitado no preço não é um preco_moedas númerico inteiro positivo.");
			$prosseguir = false;
		}

		if(!is_numeric($preco_alt) || $preco_alt < 0) {
			$form_return .= aviso_red("O Preço flocos digitado no preço não é um preco_moedas númerico inteiro positivo.");
			$prosseguir = false;
		}
		...
		if($prosseguir) {
			...
			$upload = new Upload($conn, $up_gallery, $up_file, $up_url, 'hm-', true, '', false, '', false, false);

			if(!$upload->erro) {
				$caminho_imagem = $upload->caminho;
			} else {
		...
		if($prosseguir) {
			...
			$insert_data['imagem'] = $caminho_imagem;
			$insert_data['preco_moedas'] = $preco_moedas;
			...
			$insert_data['disponivel'] = $disponivel;

			$insert = $sqlActions->insert($mdl_tabela, $insert_data);

			if($insert) {
				$core->logger("O usuário adicionou um novo item no shop.", "acao");

				$form_return .= aviso_green("Sucesso!");
				foreach($_POST as $nome_campo => $preco_moedas){ $_POST[$nome_campo] = '';}
			} else {
	...
	<div class="box-content">
		...
		$form->createInput('Nick do vendedor', 'text', 'autor',$dados['nick']);
		$form->createInput('Preço Moedas (números)', 'text', 'preco_moedas', '', 'w-md');
		$form->createInput('Preço Flocos (números)', 'text', 'preco_alt', '', 'w-md');

		$form->createInput('Quantidade disponivel (números)', 'text', 'qtd_disponivel','','w-md');

		$gratis = array();
		$gratis[] = array("label" => "Sim", "value" => 's');
		$gratis[] = array("label" => "Não", "value" => 'n');
		$form->createSelect('Gratis', 'gratis', $gratis, '', 'w-md');
		$form->createSelect('Disponível para compra', 'disponivel', $gratis, '', 'w-md');
	...
	<? if($_GET['a'] == 2) {
		...
		if($_POST['form'] == 'form') {
			...
			if(!is_numeric($preco_moedas) || $preco_moedas < 0) {
				$form_return .= aviso_red("O preco_moedas digitado no preço não é um preco_moedas númerico inteiro positivo.");
				$prosseguir = false;
			}
			...
			if($prosseguir) {
				...
				$upload = new Upload($conn, $up_gallery, $up_file, $up_url, 'hm-', false, $ex['imagem'], false, '', false, false);
			...
			if($prosseguir) {
				...
				$update_data['preco_moedas'] = $preco_moedas;
				...
				$update_data['disponivel'] = $disponivel;
				$where_data['id'] = $id;

				$update = $sqlActions->update($mdl_tabela, $update_data, $where_data);

				if($update) {
					$core->logger("O usuário editou o shop [#$id].", "acao");
		...
		<div class="box-content">
			...
			<?
				...
				$form->createInput('Nick do vendedor', 'text', 'autor',$ex['autor']);
				$form->createInput('Preço Moedas (números)', 'text', 'preco_moedas',$ex['preco_moedas'], 'w-md');
				$form->createInput('Preço Flocos (números)', 'text', 'preco_alt',$ex['preco_alt'], 'w-md');

				$form->createInput('Quantidade disponivel (números)', 'text', 'qtd_disponivel',$ex['qtd_disponivel'],'w-md');

				$gratis = array();
				$gratis[] = array("label" => "Sim", "value" => 's');
				$gratis[] = array("label" => "Não", "value" => 'n');
				$form->createSelect('Gratis', 'gratis', $gratis,  $ex['gratis'], 'w-md');
				$form->createSelect('Disponível para compra', 'disponivel', $gratis, $ex['disponivel'], 'w-md');
		...
		<? if($_GET['a'] == 3) {
			...
			if(count($ids) > 0) {
				...
				foreach ($ids as $id_atual) {
					...
					$core->logger("O usuário inativou o colante [#$id_atual]", "acao");
				}
			} else {
				$delete_where['id'] = $id;
				$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
				...
				$core->logger("O usuário inativou o colante [#$id]", "acao");
			}
		} ?>
		...
		<div class="box-content">
			...
			$table->head(array('#', 'Nome', 'Categoria', 'Valor', 'Ações'));
			...
			while($sql2 = $sql->fetch()) {
				$sql3 = $conn->query("SELECT * FROM $mdl_tabela");
				$sql4 = $sql3->fetch();
				$table->insertBody(array($sql2['id'], $core->clear($sql2['nome']), $sql4['nome'], $core->clear($sql2['preco_moedas']) . ' moedas', 'actions'), $sql2['status']);
			}
