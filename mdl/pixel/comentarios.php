<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			$update_data['comentario'] = $conteudo;
			$update_data['voto'] = $voto;
			$where_data['id'] = $id;
			$update = $sqlActions->update($mdl_tabela, $update_data, $where_data);

			if($update) {
				$core->logger("O usuário editou o comentário de arte [#$id].", "acao");
...
<div class="box-content">
	...
	$form->createTextarea('Conteúdo', 'conteudo', $ex['comentario']);

	$voto = array(
		array("label" => "Positivo", "value"=> 'pos'),
		array("label" => "Negativo", "value" => 'neg'),
		);

	$form->createSelect('Voto', 'voto', $voto, $ex['voto']);
...
<? if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário deletou o comentário de arte [#$id_atual]", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$core->logger("O usuário deletou o comentário de arte [#$id]", "acao");
	}
} ?>
...
<div class="box-content">
	...
	$table->head(array('#', 'Conteúdo', 'Arte', 'Autor', 'Data', 'Ações'));
	...
	if($_POST['search'] == 'search') {
		...
		$query = "$mdl_tabela WHERE autor LIKE ? OR comentario LIKE ? ORDER BY id DESC";
	...
	while($sql2 = $sql->fetch()) {
		$sql3 = $conn->prepare("SELECT titulo, id FROM pixel_artes WHERE id = ?");
		$sql3->bindValue(1, $sql2['id_arte']);
		$sql3->execute();
		$sql4 = $sql3->fetch();
		$table->insertBody(array($sql2['id'], $core->clear($core->encurtar($sql2['comentario'], 50)), '<a href="/idpixel/artes/'.$sql4['id'].'/'.$core->trataurl($sql4['titulo']).'">'.$core->clear($sql4['titulo']).'</a>', $core->clear($sql2['autor']), $core->clear(date('d/m/Y H:i', $sql2['data'])), 'actions'), $sql2['status']);
	}
