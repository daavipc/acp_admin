<? if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			if($insert) {
				$core->logger("O usuário adicionou uma nova categoria do iDPixel.", "acao");
...
<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			if($update) {
				$core->logger("O usuário editou a categoria do iDPixel [#$id].", "acao");
...
<? if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário deletou a categoria do iDPixel [#$id_atual]", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$core->logger("O usuário deletou a categoria do iDPixel [#$id]", "acao");
	}
} ?>
...
<div class="box-content">
	...
	$table->head(array('#', 'Nome', 'Artes postadas', 'Data', 'Ações'));
	...
	while($sql2 = $sql->fetch()) {
		$sql3 = $conn->query("SELECT count(id) FROM pixel_artes WHERE cat_id='".$sql2['id']."'");
