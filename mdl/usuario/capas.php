<? if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		$pago = $core->clear($_POST['pago']);
		$prosseguir = true;

		(($vip == 'on')) ? $vip = 's' : $vip = 'n';
		(($pago == 'on')) ? $pago = 's' : $pago = 'n';
		...
		if($prosseguir) {
			...
			$upload = new Upload($conn, $up_gallery, $up_file, $up_url, 'covers-', true);
		...
		if($prosseguir) {
			...
			if($insert) {
				$core->logger("O usuário adicionou uma nova capa de usuário.", "acao");
...
<div class="box-content">
	...
	$form->createCheckbox('Fundo VIP', 'vip');
	$form->createCheckbox('Fundo pago', 'pago');
...
<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			$upload = new Upload($conn, $up_gallery, $up_file, $up_url, 'cover-', false, $ex['imagem']);
		...
		if($prosseguir) {
			...
			if($update) {
				$core->logger("O usuário editou a capa de usuário [#$id].", "acao");
...
<div class="box-content">
	...
	$form->createCheckbox('Fundo VIP', 'vip', ($ex['vip'] == 's') ? true : false);
	$form->createCheckbox('Fundo pago', 'pago', ($ex['pago'] == 's') ? true : false);
...
<? if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário deletou a capa de usuário [#$id_atual]", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$core->logger("O usuário deletou a capa de usuário [#$id]", "acao");
	}
} ?>
