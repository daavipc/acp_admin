<? if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		$tipo = $core->clear($_POST['tipo']);
		$prosseguir = true;

		if($tipo == 1) { $tipo = 'colante'; }
		if($tipo == 2) { $tipo = 'fundo'; }

		if(!is_numeric($preco_ids) || $preco_ids < 0) {
			$form_return .= aviso_red("O preco_ids digitado no preço não é um preco_ids númerico inteiro positivo.");
			$prosseguir = false;
		}
		...
		if($prosseguir) {
			...
			if($tipo == 'colante') {
				$pasta = 'home_arquivos/home_colantes/uploads/';
			} else {
				$pasta = 'home_arquivos/home_colantes/bg/';
			}

			$upload = new Upload($conn, $up_gallery, $up_file, $up_url, 'hm-');
		...
		if($prosseguir) {
			...
			$insert_data['cat_id'] = $_POST['categoria'];
			$insert_data['gratis'] = 'n';

			$insert = $sqlActions->insert($mdl_tabela, $insert_data);

			if($insert) {
				$core->logger("O usuário adicionou um novo item na loja da Home.", "acao");

				$form_return .= aviso_green("Sucesso!");
				foreach($_POST as $nome_campo => $preco_ids){ $_POST[$nome_campo] = '';}
			} else {
	...
	<div class="box-content">
		...
		$form->createInput('Preço (números)', 'text', 'preco_ids', '', 'w-md');

		$categorias = array();
		$sql = $conn->query("SELECT * FROM hm_colantes_cat ORDER BY id ASC");
	...
	<? if($_GET['a'] == 2) {
		...
		if($_POST['form'] == 'form') {
			...
			if($prosseguir) {
				...
				$upload = new Upload($conn, $up_gallery, $up_file, $up_url, 'hm-', false, $ex['imagem'], false, '', false, false, $pasta);
			...
			if($prosseguir) {
				...
				$update_data['cat_id'] = $_POST['categoria'];
				$update_data['gratis'] = 'n';
				$where_data['id'] = $id;
				...
				if($update) {
					$core->logger("O usuário editou o colante [#$id].", "acao");
		...
		<div class="box-content">
			...
			$form->createInput('Preço (números)', 'text', 'preco_ids', $ex['preco_ids'], 'w-md');
			...
			$status_opt = array();
			$status_opt[] = array("label" => "Colante", "value" => 1);
			$status_opt[] = array("label" => "Fundo", "value" => 2);
		...
		<div class="box-content">
			...
			while($sql2 = $sql->fetch()) {
				$sql3 = $conn->query("SELECT nome FROM hm_colantes_cat WHERE id='".$sql2['cat_id']."'");
				$sql4 = $sql3->fetch();
				$table->insertBody(array($sql2['id'], $core->clear($sql2['nome']), $sql4['nome'], $core->clear($sql2['preco_ids']) . ' moedas', 'actions'), $sql2['status']);
			}
