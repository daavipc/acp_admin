<? if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		$horario_inicio = $core->clear($_POST['horario_inicio']);
		$horario_termino = $core->clear($_POST['horario_termino']);
		...
		if(empty($horario_inicio)) {
			...
			$a = explode('/', $horario_inicio);
			$b = substr($horario_inicio, -5);
			$c = explode(':', $b);
			...
			$horario_inicio = mktime($hora, $minuto, 0, $mes, $dia, $ano);
		}

		if(empty($horario_termino)) {
			$horario_termino = 0;
		} else {
			$a = explode('/', $horario_termino);
			$b = substr($horario_termino, -5);
			$c = explode(':', $b);
			...
			$horario_termino = mktime($hora, $minuto, 0, $mes, $dia, $ano);
		}

		if($prosseguir) {
			$insert_data['horario_inicio'] = $horario_inicio;
			$insert_data['horario_fim'] = $horario_termino;
			$insert_data['id_programa'] = $programa;
			...
			if($insert) {
				$core->logger("O usuário adicionou um novo horário na rádio.", "acao");
...
<div class="box-content">
	...
	$form->createInput('Horário de início', 'text', 'horario_inicio', '', 'datetimepicker');
	$form->createInput('Horário de término', 'text', 'horario_termino', '', 'datetimepicker');

	$categorias = array();
	$sql = $conn->query("SELECT * FROM radio_programas ORDER BY id ASC");
	while($sql2 = $sql->fetch()) {
		$atual = array("label" => $core->clear($sql2['nome']), "value" => $sql2['id']);
		$categorias[] = $atual;
	}

	$form->createSelect('Programa', 'programa', $categorias, '', 'w-md');
...
<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			$update_data['horario_inicio'] = $horario_inicio;
			$update_data['horario_fim'] = $horario_termino;
			$update_data['id_programa'] = $programa;

			$where_data['id'] = $id;
			$update = $sqlActions->update($mdl_tabela, $update_data, $where_data);

			if($update) {
				$core->logger("O usuário editou o horário da rádio [#$id].", "acao");
...
<div class="box-content">
	...
	$form->createInput('Horário de início', 'text', 'horario_inicio', date('d/m/Y H:i', $ex['horario_inicio']), '', '', 'Use o modelo: <pre>DD/MM/AAAA HH:MM</pre> - Ex: <pre>03/07/2015 15:00</pre><br>Use o Horário de Brasília.');
	$form->createInput('Horário de término', 'text', 'horario_termino', date('d/m/Y H:i', $ex['horario_fim']), '', '', 'Use o modelo: <pre>DD/MM/AAAA HH:MM</pre> - Ex: <pre>03/07/2015 15:00</pre><br>Use o Horário de Brasília.');
	...
	$form->createSelect('Programa', 'programa', $categorias, $ex['id_programa'], 'w-md');
...
<? if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário inativou o horário da rádio [#$id_atual]", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$core->logger("O usuário inativou o horário da rádio [#$id]", "acao");
	}
} ?>
...
<div class="box-content">
	...
	$table->head(array('#', 'Nome do programa', 'Horário de início', 'Horário de término', 'Ações'));
	...
	$query = "$mdl_tabela ORDER BY horario_inicio ASC";
	...
	if($_POST['search'] == 'search') {
		...
		$query = "$mdl_tabela WHERE $campo LIKE ? ORDER BY horario_inicio ASC";
	...
	while($sql2 = $sql->fetch()) {
		$sql3 = $conn->query("SELECT * FROM radio_programas WHERE id = " . $sql2['id_programa']);
		$sql4 = $sql3->fetch();

		$table->insertBody(array($sql2['id'], $core->clear($sql4['nome']), $core->clear(date('d/m/Y H:i', $sql2['horario_inicio'])), $core->clear(date('d/m/Y H:i', $sql2['horario_fim'])), 'actions'), $sql2['status']);
	}
