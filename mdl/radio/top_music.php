<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		$artista = $core->clear($_POST['artista']);
		$musica = $core->clear($_POST['musica']);
		$genero = $core->clear($_POST['genero']);
		$prosseguir = true;

		if(empty($artista) || empty($musica) || empty($genero)) {
		...
		if($prosseguir) {
			...
			$upload = new Upload($conn, $up_gallery, $up_file, $up_url, 'music-', false, $ex['imagem']);
		...
		if($prosseguir) {
			$update_data['artista'] = $artista;
			$update_data['musica'] = $musica;
			...
			if($update) {
				$core->logger("O usuário editou a música da Top Music [#$id].", "acao");
...
<div class="box-content">
	...
	$form->createInput('Artista', 'text', 'artista', $ex['artista']);
	$form->createInput('Música', 'text', 'musica', $ex['musica']);
	$form->createInput('Gênero', 'text', 'genero', $ex['genero']);
...
<? if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário deletou a música da Top Music [#$id_atual]", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$core->logger("O usuário deletou a música da Top Music [#$id]", "acao");
	}
} ?>
...
<div class="box-content">
	...
	$table->head(array('#', 'Artista e nome', 'Imagem', 'Posição'));
	...
	if($_POST['search'] == 'search') {
		...
		$query = "$mdl_tabela WHERE $campo LIKE ? ORDER BY id ASC";
	...
	while($sql2 = $sql->fetch()) {
		$table->insertBody(array($sql2['id'], $core->clear($sql2['artista'] . ' - ' . $sql2['musica']), '<img src="'.$core->clear($sql2['imagem']).'">', $core->clear($sql2['posicao']) . 'ª'), $sql2['status']);
	}
