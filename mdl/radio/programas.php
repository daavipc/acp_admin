<? if($_GET['a'] == 1) {
	if($_POST['form'] == 'form') {
		...
		$locutor = $core->clear($_POST['locutor']);
		$prosseguir = true;

		if(empty($nome) || empty($locutor)) {
		...
		if($prosseguir) {
			...
			if($insert) {
				$core->logger("O usuário adicionou um novo programa na rádio.", "acao");
...
<div class="box-content">
	...
	$form->createInput('Locutor', 'text', 'locutor', $dados['nick']);
...
<? if($_GET['a'] == 2) {
	...
	if($_POST['form'] == 'form') {
		...
		if($prosseguir) {
			...
			$update_data['locutor'] = $locutor;

			$where_data['id'] = $id;
			$update = $sqlActions->update($mdl_tabela, $update_data, $where_data);

			if($update) {
				$core->logger("O usuário editou o programa da rádio [#$id].", "acao");
...
<div class="box-content">
	...
	$form->createInput('Locutor', 'text', 'locutor', $ex['locutor']);
...
<? if($_GET['a'] == 3) {
	...
	if(count($ids) > 0) {
		...
		foreach ($ids as $id_atual) {
			...
			$core->logger("O usuário inativou o programa da rádio [#$id_atual]", "acao");
		}
	} else {
		$delete_where['id'] = $id;
		$delete = $conn->prepare("UPDATE $mdl_tabela SET status = 'inativo' WHERE id = ? LIMIT 1");
		...
		$core->logger("O usuário inativou o programa da rádio [#$id]", "acao");
	}
} ?>
...
<div class="box-content">
	...
	$table->head(array('#', 'Nome', 'Locutor', 'Data de criação', 'Ações'));
	...
	while($sql2 = $sql->fetch()) {
		$table->insertBody(array($sql2['id'], $core->clear($sql2['nome']), $core->clear($sql2['locutor']), $core->clear(date('d/m/Y H:i', $sql2['data'])), 'actions'), $sql2['status']);
	}
